<?php
//https://github.com/fernanACM/GrapplingHook/blob/6781419f7867743f4ca2538d43c4dc0bb04d69fb/src/fernanACM/GrapplingHook/FishingHook.php
...
class GraplingHookEntity extends Throwable {
	...
	public function __construct(
		...
		if ($owner instanceof Player) {
			$this->setPosition(
				$this->location->add(0, $owner->getEyeHeight() - 0.1, 0)
			);
			$this->setMotion($owner->getDirectionVector()->multiply(0.4));
			GraplingHook::setFishingHook($this, $owner);
			$this->handleHookCasting(
				$this->motion->x,
				$this->motion->y,
				$this->motion->z,
				1.5,
	...
	public function handleHookCasting(
		float $x,
		float $y,
		float $z,
		float $f1,
	...
	public function entityBaseTick(int $tickDiff = 1): bool {
		...
		if ($owner instanceof Player) {
			if (
				!$owner->getInventory()->getItemInHand() instanceof
					GraplingHook or
				!$owner->isAlive() or
				$owner->isClosed()
			) {
	...
	public function onDispose(): void {
		...
		$owner = $this->getOwningEntity();
		if ($owner instanceof Player) {
			GraplingHook::setFishingHook(null, $owner);
		}
	}
	...
	public function handleHookRetraction(): void {
		...
		$owner->setMotion(
			$this->location
				->subtractVector($ownerPos)
				->multiply($this->getGrapplingSpeed($dist))
		);
		$this->flagForDespawn();
	}
