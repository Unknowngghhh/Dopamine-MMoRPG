...
class ArachneChestplate extends SkyblockArmor implements ItemComponents {
	...
	public function __construct(
		...
		parent::__construct(
			$identifier,
			$name,
			new SkyBlockArmorInfo(4, ArmorInventory::SLOT_CHEST)
		);
		...
		$this->properties->setType(
			SkyblockItemProperties::ITEM_TYPE_ARMOR
		);

		$this->addComponent(new ArmorComponent(4, 'leather'));
	}
	...
	public function onTransaction(
		...
		if (
			$itemClickedWith instanceof ArachneChestplate &&
			$itemClicked instanceof ArachneChestplate
		) {
			$stacked = $itemClicked
				->getNamedTag()
